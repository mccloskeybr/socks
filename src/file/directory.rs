...
fn find_directory_entry<F: Read + Write + Seek>(
    index: &mut Index<F>,
    id: u32,
    ...
    for i in 0..index.metadata.num_directories {
        ...
        let mut dir_chunk =
            chunk::read_chunk_at::<F>(&index.db_config.file, &mut index.file, dir_chunk_offset)?;
...
pub fn find_chunk_offset<F: Read + Write + Seek>(
    index: &mut Index<F>,
    id: u32,
) -> Result<u32, Error> {
...
pub fn update_chunk_offset<F: Read + Write + Seek>(
    index: &mut Index<F>,
    id: u32,
    offset: u32,
) -> Result<(), Error> {
...
pub fn create_directory_entry<F: Read + Write + Seek>(
    index: &mut Index<F>,
    id: u32,
    offset: u32,
) -> Result<(), Error> {
    ...
    if let Some((mut dir_chunk, dir_chunk_offset)) = dir_chunk {
        log::trace!(
            "Found a directory with existing space: {}",
            dir_chunk_offset
        );
        ...
        let swap_chunk =
            chunk::read_chunk_at::<F>(&index.db_config.file, &mut index.file, new_dir_offset)?;
